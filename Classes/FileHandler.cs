...
namespace Helion
{
  internal sealed class FileHandler
  {
    ...
    public FileHandler(string showTitel, string seasonNr, string fileExtension)
    {
    ...
    public static List<string[]> CreateDataGridFilePreview(string showTitel, string seasonNr, string fileExtension)
    {
      ...
      return new FileHandler(showTitel, seasonNr, fileExtension).CreateDataGridPreviewList();
    }
    ...
    public static void RenameFilesFromList(string showTitel, string seasonNr, string fileExtension)
    {
      ...
      new FileHandler(showTitel, seasonNr, fileExtension).RenameFilesFromListData();
    }
    ...
    private List<string[]> CreateDataGridPreviewList()
    {
      ...
      string[] episodeNames = File.ReadAllLines(MainWindow.ListTxtFilePath);
      var mainWindow = Application.Current.Windows.OfType<MainWindow>().FirstOrDefault();
      if (mainWindow == null) return result;
      FileInfo[] infos = new DirectoryInfo(MainWindow.ApplicationDirectory).GetFiles();
    ...
    private void RenameFilesFromListData()
    {
      ...
      for (int i = 0; i < loopLimit; i++)
      {
        ...
        string newName = $"{mainWindow.RetrieveFileNamePattern()
          ...
          .Replace("{EpisodeNumber}", "E" + episodeNumber)
          .Replace("{EpisodeName}", episodeNames[i])}{FileExtension}";
        string oldName = Path.Combine(MainWindow.ApplicationDirectory, filteredStrings[i]);
        string newFileName = Path.Combine(MainWindow.ApplicationDirectory, newName);
        try
        {
